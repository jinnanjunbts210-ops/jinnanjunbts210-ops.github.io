...
const BUILTIN_PROBLEMS = [
  ...
  },
  {
    ...
    description: [
      ...
      "样例 1",
      "输入：\n`charMatrix = [\"ECVR\", \"ABNH\", \"MBYH\"]`\n`words = [\"EC\", \"CB\", \"BNH\", \"ECBN\", \"RHHY\", \"VB\", \"ECE\"]`",
      "输出：\n`5`",
      "解释：`EC`、`CB`、`BNH`、`ECBN` 和 `RHHY` 可以匹配；`VB` 不是上下左右相邻，`ECE` 需要重复使用唯一的 E 格子，因此不能匹配。",
      "样例 2",
      "输入：\n`charMatrix = [\"AC\", \"EX\"]`\n`words = [\"XE\", \"ACX\", \"AX\"]`",
      "输出：\n`2`",
      ...
      "批量输入的第一行是测试数量 T，之后每组占两行：第 1 行为 `charMatrix`，第 2 行为 `words`。运行结果必须依次为 `5、4、2、4、1、5、1` 才会通过样例检测。",
      "你只需要完成 `Solution::GetMatchedWordCount`，系统会自动注入 `main` 和本地测试框架。",
    ].join("\n\n"),
    sampleInput: [
      "7",
      "[\"ECVR\", \"ABNH\", \"MBYH\"]",
      "[\"EC\", \"CB\", \"BNH\", \"ECBN\", \"RHHY\", \"VB\", \"ECE\"]",
      "[\"ABCD\", \"EFGH\", \"IJKL\"]",
    ...
    sampleOutput: ["5", "4", "2", "4", "1", "5", "1"].join("\n"),
    starterCode: [
      "#include <string>",
      "#include <vector>",
      "",
